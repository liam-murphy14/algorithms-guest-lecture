...
\Procedure{Is-Valid}{$x,y,n$}
	...
	\Return{true}
\EndProcedure
\Procedure{Is-Safe-From-Bishop}{$x,y,A$}
	\If{$(x,y)$ in $A$}
		\Return{false}
	\EndIf
	\Return{true}
	...
	\While{$Q\neq\varnothing$}
		...
		\If{$v\notin visited$}
			...
			\For{$i\in\set{1,2,\ldots,8}$}
				...
				\If{$b=true$}
					\If{\Call{Is-Valid}{$x_{new},y_{new},n$} and \Call{Is-Safe-From-Bishop}{$x_{new},y_{new},A$}}
...
\end{algorithm}
\section*{Analysis}
To analyze this solution, we will use a slightly different method than what you may be used to. Instead of expressing the complexity in terms of the input chess board size, $n$, we will express it in terms of the distance between the start and end positions. This will help us understand more accurately how long the algorithm will take to run, which is particularly important for coding interviews.\\
